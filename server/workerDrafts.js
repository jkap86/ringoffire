...
const getDrafts = async (season) => {
    const leagues = await axios.get(`http://api.sleeper.app/v1/user/435483482039250944/leagues/nfl/${season}`)
    const leaguesROF = leagues.data.filter(x => x.name.includes('Ring of Fire: '))
    const draftsROF = []
    await Promise.all(leaguesROF.map(async league => {
        const league_drafts = await axios.get(`http://api.sleeper.app/v1/league/${league.league_id}/drafts`)
        const users = await axios.get(`http://api.sleeper.app/v1/league/${league.league_id}/users`)
        await Promise.all(league_drafts.data.map(async draft => {
            const [draft_info, draft_picks] = await Promise.all([
                await axios.get(`http://api.sleeper.app/v1/draft/${draft.draft_id}`),
                await axios.get(`http://api.sleeper.app/v1/draft/${draft.draft_id}/picks`)
            ])
            draftsROF.push({
